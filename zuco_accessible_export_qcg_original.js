...
login().then(() => { //END OF REDIRECT LOG IN CODE
	require.config( {
...
/* 		app.visualization.get('arrXTE').then(function(vis){
		...
		app.getObject('simple_table','arrXTE').then(function(vis) {
		  //$('#cmdExport').show();
		  $('#cmdExport').click(function() {
			 vis.exportData({format:'CSV_T', state: 'A'}).then(function(link) {
				window.open(link);
		...
		//ACCESS IS DENIED MESSAGE
/* 		$('#cmdExport').click(function(e){
             app.getObject('arrXTE').then(function(model) {
              var table = qlik.table(model);
